  ...
  return (
    <div
      className="max-w-md h-[450px] p-5 text-white transform scale-100 transition-transform duration-300 rounded-2xl
                 ...
                 shadow-xl hover:shadow-emerald-500/20 hover:scale-105 hover:-translate-y-1 hover:border-emerald-500/50"
    ...
    >
      ...
      <div className="mt-2 overflow-y-auto h-[170px] pr-2 scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-slate-800/50">
        ...
        <div className="flex items-center mb-2 space-x-2.5">
          <h1 className="text-2xl font-bold text-transparent bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text">
            {titulo}
          </h1>
          <a href={enlace} target="_blank" rel="noopener noreferrer">
            <FaUpRightFromSquare className="mt-1 transition-colors text-slate-400 hover:text-emerald-400" />
          </a>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {tecnologias.map((tech, index) => (
            <div
              key={index}
              className="flex items-center px-3 py-1.5 space-x-2 text-xs font-semibold text-emerald-100 
                         bg-gradient-to-r from-slate-700/80 to-slate-800/80 backdrop-blur-sm 
                         rounded-full border border-slate-600/50 hover:border-emerald-500/50 transition-colors"
            >
              {tech.icono}
